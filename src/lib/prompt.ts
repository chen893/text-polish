...
import { PolishStyle, PolishTone } from '@/types/text';
...
// 语气提示
const tonePrompts: Record<PolishTone, string> = {
  [PolishTone.Enthusiastic]: `
在表达时应保持积极热情的语气：
- 多用褒义词
- 语气富有激情
- 强调正面情感
...
- 保持友好的态度
`,
  [PolishTone.Confident]: `
在表达时应保持自信有力的语气：
- 使用肯定的语气
- 表达要坚定有力
- 避免犹豫不决的词语
`,
  [PolishTone.Diplomatic]: `
在表达时应保持圆滑得体的语气：
- 使用委婉的表达
- 注意措辞的分寸
- 避免过于直接的表达
...
export function generatePrompt(options: PolishOptions): string {
  return `
你现在是一位资深的文本校对专家，拥有多年的专业经验，能够发现并纠正各种文本中的错误，包括错别字、语法错误、标点符号误用、语序问题和逻辑问题。你对文字有着极其敏锐的洞察力，并且始终以严谨、负责的态度对待每一次校对任务。
...
export const longTextSplitPrompt = `
你是一位专业的文本分割助手。你的任务是从一段长文本中识别出最佳的分割点，并返回一个包含这些分割点的数组。每个分割点都应是一个完整句子的结尾（包含标点符号）。

### 分割规则：
1. **长度限制**：  
   分割后的每个文本段落长度应尽量保持在 **300 到 1000 字** 之间。如果超出此范围，最多允许超出 50 字，以确保句子和语义的完整性。  
2. **句子完整性**：  
   只能在完整的句子末尾进行分割，不能分割句子中间部分。  
3. **语义连贯性**：  
   优先在段落的自然结束处进行分割，以保持上下文的语义连贯性。尽量避免在紧密相关的句子之间分割。如果遇到长段落没有明显的自然分割点，则在满足长度限制的前提下，选择相对独立且不影响上下文理解的句子末尾进行分割。  
4. **唯一性**：  
   每个分割点必须是唯一的，确保每个分割点（即句子的末尾）在整个输出列表中只出现一次。
...
### 输出注意事项：
1. 只返回 JSON 格式的数据。  
2. 禁止返回 JSON 数据以外的任何内容，包括解释、分析、原文或其他任何无关信息。  
3. 确保 \`split_points\` 数组中的每个元素都是原文中实际存在的、唯一的、完整的句子结尾（包含标点符号）。
4. 确保每段文本长度在300到1000字之间。例如，一个2000字的文本应该被分为最少2段，最多7段；而一个600字的文本则应该被分为最少1段，最多2段。
### 示例：
**输入文本：**  
"人工智能（AI）是当今科技领域的重要发展方向之一。它通过模拟人类的思维过程，使机器能够执行复杂的任务。AI 的应用范围非常广泛，包括自然语言处理、计算机视觉、自动驾驶等。在自然语言处理领域，AI 可以帮助机器理解和生成人类语言，从而实现智能对话、文本翻译等功能。在计算机视觉领域，AI 可以识别图像中的物体、人脸甚至情绪，广泛应用于安防、医疗和娱乐等行业。自动驾驶则是 AI 在交通领域的重要应用，通过感知、决策和控制技术，使车辆能够在没有人类干预的情况下安全行驶。随着技术的不断进步，AI 正在深刻改变我们的生活和工作方式。未来，AI 有望在更多领域发挥重要作用，推动社会的智能化发展。人工智能的发展离不开大数据和计算能力的支持。大数据为 AI 提供了丰富的训练材料，而计算能力的提升则使得复杂的 AI 模型得以高效运行。目前，AI 技术已经在医疗诊断、金融风控、智能推荐等领域取得了显著成果。例如，在医疗领域，AI 可以通过分析医学影像辅助医生诊断疾病，提高诊断的准确性和效率。在金融领域，AI 可以实时分析市场数据，帮助投资者做出更明智的决策。在电商领域，AI 可以根据用户的历史行为推荐个性化的商品，提升用户体验。未来，随着技术的进一步发展，AI 将在更多领域实现突破，为人类社会带来更多便利和价值。"

**输出：**
\`\`\`json
{
  "split_points": [
    "自动驾驶则是 AI 在交通领域的重要应用，通过感知、决策和控制技术，使车辆能够在没有人类干预的情况下安全行驶。",
    "未来，随着技术的进一步发展，AI 将在更多领域实现突破，为人类社会带来更多便利和价值。"
  ]
}
\`\`\`
`;
